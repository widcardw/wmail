...
const EmailContent: Component<{ email: Email }> = (props) => {
  ...
  return (
    <main>
      <header class="p-4 flex gap-4 bg-bg" style={{'border-bottom': '1px solid var(--color-border)'}}>
        <div class={clsx("w-12 h-12 rounded-full text-primary-fg bg-primary",
        ...
        <div class="min-w-0 flex-1">
          <div class="flex items-start justify-between gap-4">
            ...
            <span class="text-sm text-mut-foreground whitespace-nowrap">{fullDate(props.email.date)}</span>
          </div>
      ...
      <pre class="p-4" style="white-space: pre-wrap; word-break: break-word;">{props.email.body}</pre>
    </main>
  )
}
